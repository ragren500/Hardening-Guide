...
## 1. **Övervaka system på Windows**

### 1.1. **Windows Defender Antivirus**
Windows Defender är det inbyggda antivirusprogrammet på Windows. Det övervakar systemet för att upptäcka och blockera skadlig kod och virus.

#### Så här använder du Windows Defender:
1. Gå till **Inställningar** > **Uppdatering och säkerhet** > **Windows-säkerhet**.
2. Klicka på **Virus- och hotdetektering** för att starta en fullständig skanning.
3. Du kan också konfigurera **Windows Defender** att köra en automatisk skanning.

### 1.2. **Event Viewer**
Event Viewer är ett inbyggt verktyg som loggar alla systemhändelser och kan användas för att övervaka säkerhetsrelaterade händelser.

#### Så här använder du Event Viewer:
1. Skriv "Event Viewer" i sökfältet och öppna programmet.
2. Gå till **Windows Logs** > **Security** för att se säkerhetsloggar.
3. Du kan skapa filter för att fokusera på specifika säkerhetsrelaterade händelser, såsom inloggningar eller felaktiga inloggningsförsök.

### 1.3. **Sysmon (System Monitor)**
Sysmon, från Sysinternals, loggar detaljerad information om systemhändelser och nätverkstrafik.

#### Så här använder du Sysmon:
1. Ladda ner **Sysmon** från [Sysinternals](https://docs.microsoft.com/en-us/sysinternals/downloads/sysmon).
2. Installera och konfigurera Sysmon för att logga aktivitet som systemändringar, nätverksanslutningar och filåtkomst.
3. Sysmon-loggar kan ses i Event Viewer under **Applications and Services Logs** > **Microsoft** > **Windows** > **Sysmon**.
